...
class Payment(models.Model):
    ...
    currency = models.CharField(max_length=3, default='KZT')  # RUB, USD и т.д.
    order_id = models.CharField(max_length=100, unique=True, default=999)
    description = models.TextField(blank=True)
    status = models.CharField(max_length=20, default='pending')
    created_at = models.DateTimeField(auto_now_add=True)
    paybox_payment_id = models.CharField(max_length=100, blank=True)

    def __str__(self):
        return f"Payment #{self.order_id} - {self.amount}"
